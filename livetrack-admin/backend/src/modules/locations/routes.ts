...
locationsRouter.get("/latest",async(req,res)=>{const devices=await prisma.device.findMany({where:req.user!.role==="ADMIN"||req.user!.role==="VIEWER"?{}:{userId:req.user!.id},include:{user:{select:{fullName:true}},locations:{orderBy:{recordedAt:"desc"},take:1}}});res.json({locations:devices.filter(d=>d.locations[0]).map(d=>({...d.locations[0],device:{id:d.id,deviceName:d.deviceName,owner:d.user.fullName,isTracking:d.isTracking,lastSeenAt:d.lastSeenAt},status:getStatus(d.lastSeenAt)}))});});
