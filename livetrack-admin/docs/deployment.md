...
Terminate HTTPS at a trusted load balancer or reverse proxy. Forward WebSocket upgrade headers for `/socket.io`, set the exact HTTPS dashboard origin in `CLIENT_URL`, rotate both JWT secrets, disable demo data, and run Prisma deploy as a controlled release step.
